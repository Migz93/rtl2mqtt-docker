...
while True:
    ...
    for line in iter(rtl433_proc.stdout.readline, '\n'):
        ...
        if "time" in line:
            mqttc.publish(MQTT_TOPIC, payload=line, qos=MQTT_QOS, retain=MQTT_RETAIN)
            ...
            for item in json_dict:
                ...
                if not "model" in item:
                    mqttc.publish(MQTT_TOPIC+"/"+subtopic+"/"+item, payload=value, qos=MQTT_QOS, retain=MQTT_RETAIN)
